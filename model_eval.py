...
category_dictionary["type_filter"] = ['0', '1', '2', '3']
category_dictionary["send_group_filter"] = ["defenders", "midfielders", "attackers"]
category_dictionary["receive_group_filter"] = ["defenders", "midfielders", "attackers"]
category_dictionary["x_filter"] = ["own", "other"]
category_dictionary["y_filter"] = ["center", "sides"]
...
def exit_code():
    ...
    if len(sys.argv) != 3:
        ...
        use_filter = True
        if filter_type not in category_dictionary.keys():
            print("unknown filter")
            exit()
        elif category_type not in category_dictionary[filter_type]:
            print('unknown category for filter type')
            exit()
        else:
            print('valid filters')
            #return use_filter
...
def get_filter(filter_type, filter_category):
    value = None
    if filter_type == 'type_filter':
        #filter = 'type_filter'
        type_filter = True
        value = int(category_type)

    elif filter_type == 'send_group_filter':
        send_group_filter = True
        if category_type == 'defenders':
            value = [1, 2, 3, 4, 5]
        elif category_type == 'midfielders':
            value = [6, 7, 8]
        else:
            value = [9, 10, 11]

    elif filter_type == 'receive_group_filter':
        receive_group_filter = True
        if category_type == 'defenders':
            value = [1, 2, 3, 4, 5]
        elif category_type == 'midfielders':
            value = [6, 7, 8]
    ...
    elif filter_type == 'x_filter':
        x_filter = True
        if category_type == 'own':
            value = True
        else:
            value = False
...
category_type = str(sys.argv[2])
exit_code()
value = get_filter(filter_type, category_type)
print('filter used?', use_filter)
print(filter_type)
print(category_type)
print(value)
...
def read_file():
    ...
    #file_name = '5matches_kicker_pass.csv'
    ...
    for h in header:
        ...
        if h == 'out_unum_kicker':
            out_kicker_number = counter

        if h == 'out_unum':
            out_unum_number = counter

        if h == 'out_desc':
            out_type_number = counter

        if h == 'out_or_target_x':
            out_x_number = counter

        if h == 'out_or_target_y':
            out_y_number = counter
        counter += 1
    rows = []
    line_number = 0
    for line in lines[1:]:
        ...
        if use_filter:
            if filter_type == 'type_filter':
                if f_row[out_type_number] == value:
                    rows.append(f_row)
            elif filter_type == 'receive_group_filter':
                if f_row[out_unum_number] in value:
                    rows.append(f_row)
            elif filter_type == 'send_group_filter':
                if f_row[out_unum_kicker] in value:
                    rows.append(f_row)
            elif filter_type == 'x_filter':
                if value:
                    if f_row[out_x_number] < 0:
                        rows.append(f_row)
                else:
                    if f_row[out_x_number] > 0:
                        rows.append(f_row)
            elif filter_type == 'y_filter':
                # TODO
                if value:
                    if f_row[out_y_number] < 0:
                        rows.append(f_row)
                else:
                    if f_row[out_y_number] > 0:
                        rows.append(f_row)
...
train_datas, test_datas, train_labels, test_labels = train_test_split(data_x, data_y, test_size=0.5, random_state=42)
