...
void DataExtractor::update(const PlayerAgent *agent, const ActionStatePair *first_layer,bool update_shoot) {
    ...
    // cycle
    ADD_ELEM("cycle", last_update_cycle);
...
void DataExtractor::init_file(const rcsc::WorldModel &wm) {
    ...
    std::string dir = "/storage2/dvandijk/Agent2D-DataExtractor/dataset7/";
    ...
    header += "out_category,out_unum_kicker,out_or_target_x,out_target_x,out_or_target_y,out_target_y,out_unum,out_unum_index,out_ball_speed,out_or_ball_dir,out_ball_dir,out_desc,";
...
void DataExtractor::extract_ball(const rcsc::WorldModel &wm) {
	...
	//ADD_ELEM("p_or_x", wm.ball().pos().x);
        //ADD_ELEM("p_or_y", wm.ball().pos().y);
	...
	//ADD_ELEM("p_or_x", invalid_data);
        //ADD_ELEM("p_or_y", invalid_data);
...
void DataExtractor::extract_output(const rcsc::WorldModel &wm,
    ...
    ADD_ELEM("unum_kicker", wm.self().unum());
    ADD_ELEM("or_target_x", target.x);
    ADD_ELEM("target_x", convertor_x(target.x));
    ADD_ELEM("or_target_y", target.y);
    ...
    ADD_ELEM("or_ball_dir", (target - wm.ball().pos()).th().degree());
    ...
    if (category == 2) {
        if (std::string(desc) == "strictDirect") {
            ...
            ADD_ELEM("desc", 3);
        } else {ADD_ELEM("desc", 4);}
    } else if (category == 1) {
        if (std::string(desc) == "SelfPass") {
            ADD_ELEM("desc", 0);
        } else if (std::string(desc) == "shortDribble") {
            ADD_ELEM("desc", 1);
        } else if (std::string(desc) == "queuedDribble") {
            ADD_ELEM("desc", 2);
        } else {ADD_ELEM("desc", 3);}
    } else { ADD_ELEM("desc", 4);
	}
}
...
void DataExtractor::extract_pos(const AbstractPlayerObject *player, const WorldModel &wm, DataSide side) {
	...
	//ADD_ELEM("pos_or_x", player->pos().x);
        //ADD_ELEM("pos_or_y", player->pos().y);
    }
    if (option.polarPos == side || option.polarPos == BOTH) {
